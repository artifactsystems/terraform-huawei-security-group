...
variable "rules" {
  ...
  default = {
    ...
    # Note: HuaweiCloud uses null for protocol (all protocols) and ICMP doesn't use ports
    all-all  = [null, null, "", "All protocols"]
    all-tcp  = [1, 65535, "tcp", "All TCP ports"]
    all-udp  = [1, 65535, "udp", "All UDP ports"]
    all-icmp = [null, null, "icmp", "All IPV4 ICMP"]
